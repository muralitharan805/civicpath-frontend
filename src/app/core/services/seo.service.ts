 ...
 * and GA4 Google Analytics navigation tracking across SPA route transitions for CivicPath.
 */
@Injectable({
  providedIn: 'root',
})
export class SeoService {
  ...
  private readonly gaMeasurementId = 'G-5CMPXHDLW8';
  ...
  initRouteTracking(): void {
    this.router.events
      ...
      .subscribe((event: NavigationEnd) => {
        const fullUrl = `https://civicpath.seyalicraft.com${event.urlAfterRedirects}`;
  ...
  setCanonicalUrl(url?: string): void {
    const rawUrl = url || `https://civicpath.seyalicraft.com${this.document.location?.pathname || ''}`;
